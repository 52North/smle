import { AbstractSWEIdentifiable, SweVector, SweDataArray, SweDataRecord } from '../swe';
...
export class PhysicalComponent extends AbstractPhysicalProcess {
  /**
   * he method describes (as an algorithm or text) how the process takes the input and, based on the parameter values, generates output values.
   */
...
export class TemporalFrame extends AbstractSWEIdentifiable {
  /**
   * The origin should just describe context of the start of time (e.g. start of local timer).
   */
  origin: string;
}
...
type Position = string | Point | SweVector | SweDataRecord | SweDataArray | AbstractProcess;