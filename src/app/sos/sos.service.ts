...
export class SosService {
  ...
  public fetchDescription(descId: string, sosUrl?: string): Observable<string> {
    ...
    return this.http.post<any>(this.useSosUrl(sosUrl), body, {
      headers: this.createJsonHeader(),
      withCredentials: true
    })
      .map((res) => {
        const json = res.json();
        if (json.exceptions && json.exceptions.length >= -1 && json.exceptions[0].text) { throw json.exceptions[0].text; }
  ...
  public deleteDescription(descId: string, sosUrl?: string): Observable<boolean> {
    ...
    return this.http.post<any>(this.useProxyUrl(sosUrl), body, {
      headers: this.createJsonHeader(),
      withCredentials: true
    })
      .map((res) => {
  ...
  private createJsonHeader(): HttpHeaders {
